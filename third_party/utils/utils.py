...
class TaskCollator_gemma:
    ...
    EVAL mode:
        ...
        labels_eval          = -100 * len(prompt) + target
    ...
    def __init__(self, tokenizer, data_args, tpu_num_cores=None):
        ...
        self._log_count = 0  # Track how many batches we've logged
    ...
    def __call__(self, batch):
        ...
        for p_ids, t_ids in zip(prompt_enc["input_ids"], target_enc["input_ids"]):
            ...
            train_input = p_ids + t_ids_eos[1:]
            
            train_mask  = [1] * len(train_input)

            # labels mask prompt and reveal target
            train_labels = ([self.ignore_index] *(len(p_ids))) + t_ids_eos[1:]
            ...
            # FIX #6: eval labels = target tokens only (with eos)
            # Old code forced eval_labels to match prompt length, causing truncation
            # or misaligned padding. Metrics expect target-only labels.
            eval_labels = t_ids_eos[1:]
        ...
        # --- LOG: Batch shape summary (first 3 batches only to avoid spam) ---
        if self._log_count < 3:
            n_pad_in_first = (input_ids[0] == self.pad_token_id).sum().item()
            n_ignore_in_first = (labels[0] == self.ignore_index).sum().item()
            n_response_in_first = (labels[0] != self.ignore_index).sum().item()
            print(f"[COLLATOR] task={task_name} | batch_size={input_ids.shape[0]}")
            print(f"  TRAIN: input_ids={list(input_ids.shape)}, labels={list(labels.shape)}")
            print(f"    sample[0]: {n_pad_in_first} pad tokens, {n_ignore_in_first} masked(-100), {n_response_in_first} response tokens")
            print(f"  EVAL:  input_ids_eval={list(input_ids_eval.shape)}, labels_eval={list(labels_eval.shape)}")
            # Decode first sample to verify sanity
            first_prompt = self.tokenizer.decode(input_ids_eval[0][input_ids_eval[0] != self.pad_token_id], skip_special_tokens=True)
            first_target = self.tokenizer.decode(labels_eval[0][labels_eval[0] != self.ignore_index], skip_special_tokens=True)
            print(f"    sample[0] prompt (first 100 chars): {first_prompt[:100]}...")
            print(f"    sample[0] target (first 100 chars): {first_target[:100]}...")
            
            # --- Dump arrays to txt files for debugging ---
            import numpy as np
            np.savetxt(f"train_input_{task_name}_{self._log_count}.txt", input_ids[0].cpu().numpy(), fmt='%d', header="train_input")
            np.savetxt(f"train_labels_{task_name}_{self._log_count}.txt", labels[0].cpu().numpy(), fmt='%d', header="train_labels")
            np.savetxt(f"eval_input_{task_name}_{self._log_count}.txt", input_ids_eval[0].cpu().numpy(), fmt='%d', header="eval_input")
            np.savetxt(f"eval_labels_{task_name}_{self._log_count}.txt", labels_eval[0].cpu().numpy(), fmt='%d', header="eval_labels")
            self._log_count += 1
